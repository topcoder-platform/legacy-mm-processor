...
const reviewSummationSchema = Schema.createEventSchema({
  ...
  submissionId: Joi.sid().required(),
  aggregateScore: Joi.number().min(0).max(100).required()
})
...
async function handle (value) {
  ...
  if (event.payload.resource === 'submission') {
    ...
    logger.debug(`Successfully processed MM message - Patched to the Submission API: id ${event.payload.id}, patch: ${JSON.stringify(patchObject)}`)
